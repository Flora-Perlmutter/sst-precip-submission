...
# coding: utf-8
"""
Figure 11: AMIP average SST sensitivity and SST variability maps.

Author: Flora Perlmutter

Description
-----------
AMIP counterpart to Figure 8. Identical 4-panel layout using AMIP ensemble
quantities instead of observational ensemble quantities:
  Panel A: AMIP ensemble mean SST sensitivity (dP/dSST), with
...
Output
------
  figures/paper_figures/Figure_11_average_sensitivity_importance_AMIP.png
...
# =============================
# TOP ROW: SST Forced Contribution
# =============================

# -------------------------------
# Panel A: Average SST Sensitivity
# -------------------------------
...
# -------------------------------
# Panel B: SST Forced Contribution
# -------------------------------
...
ax2.set_title("AMIP Average SST-Forced Contribution")
...
for ax, label in zip([ax1, ax2, ax3, ax4], ['a', 'b', 'c', 'd']):
    ax.text(-.075, 1.15, label, transform=ax.transAxes,
            fontsize=10, fontweight='bold', va='top')
    ax.set_aspect('auto')

plt.savefig(FIGURES_DIR / "Figure_11_average_sensitivity_importance_AMIP.png",
...
# Total ocean cells = cells with a finite SST Sensitivity value (land is NaN)
ocean_mask = np.isfinite(amip_ms)
...
sig_positive = is_significant & (amip_ms > 0)
sig_negative = is_significant & (amip_ms < 0)
...
mean_pos = float(amip_ms.where(sig_positive).mean(skipna=True).values)
mean_neg = float(amip_ms.where(sig_negative).mean(skipna=True).values)

# ----------------------------------------------------------
# Print results
# ----------------------------------------------------------
print("Significant AMIP SST Sensitivity Summary")
print("-" * 50)
