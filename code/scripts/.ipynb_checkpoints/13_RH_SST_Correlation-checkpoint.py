...
def process_pair(p_name, p_da, sst_name, sst_da, rh_da):
    ...
    # Compute correlation
    rh_sst_corr = xr.corr(rh_expanded, sst_expanded, dim='time')
    ...
    return ((p_name, sst_name), rh_sst_corr)
...
def main():
    ...
    # Compute correlation
    key, rh_sst_corr = process_pair(p_name, p_da, sst_name, sst_da, rh_da)
    
    # Save result
    output_file = os.path.join(outputs_dir, f'rh_sst_correlation_{p_name}_{sst_name}.nc')
    
    print(f"\nSaving results...")
    rh_sst_corr.to_netcdf(output_file)
