...
def run_single_bootstrap(seed, n_time, sst_detrended, p_detrended, sst_anom,
    ...
    #
    # A trend over the replicate is well defined: the resampling perturbs the
    # slope field, but the reconstruction multiplies it by the un-resampled
    # sst_anom, so the replicate lives on the original time axis. Its spread is
    # the trend uncertainty contributed by beta estimation error.
...
def bootstrap_se_incremental(sst_detrended, p_detrended, sst_anom, area_precomputed,
    ...
    # Full distributions rather than Welford summaries. A percentile interval on
    # the trend needs the replicates themselves, and at 100 x n_basins these are
    # ~200 KB each. Welford stays for the two large fields, where retaining 100
    # replicates of (lat, lon, basin) would not fit.
...
def process_pair(p_name, p_anom, sst_name, sst_anom):
    ...
    # Trend point estimates, on the same inputs as their bootstrap counterparts
...
def main():
    ...
    result_ds['trend_boot'].attrs.update({
        ...
        'note': ('Replicates share the observed SST history and differ only in '
                 'the fitted slope field, so the spread is the trend uncertainty '
                 'from beta estimation, not from SST or dataset choice.'),
    })
