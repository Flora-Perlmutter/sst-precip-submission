...
for model_id in AMIP_MODELS:
    output_file = os.path.join(
        outputs_dir,
        f'global_linear_regression_bootstrap_amip_{model_id}_{model_id}.nc'
    )
...
gs = fig.add_gridspec(nrows=1, ncols=2, hspace=0.3, wspace=0.25, width_ratios=[1.2, 1])
plt.rcParams.update({'font.size': 18})

# --- Map 1: Colored by correlation ---
ax1 = fig.add_subplot(gs[0, 0], projection=ccrs.Robinson(central_longitude=0))
ax1.set_title('AMIP vs Observed Marginal Sensitivity', fontsize=14, fontweight='bold')
ax1.set_global()
num_levels = 20
vmax=1
vmin=-1
...
n, bins, patches = ax_hist_2.hist(
    ratio_values_clean, bins=hist_bins_2, color="lightblue", edgecolor="black", alpha=0.8
)
...
for basin_name in highlight_basins_2:
    ...
    ax_hist_2.annotate(
        ...
        xytext=(text_x, bin_height + text_y + 6),
        ha=ha_text,
        arrowprops=dict(arrowstyle="->", lw=1.2, color="black"),
        fontsize=11,
        bbox=dict(facecolor='white', alpha=0.75, edgecolor='none', boxstyle="round,pad=0.2", zorder = 5),

    )
    
# Add panel label
ax_hist_2.text(-0.17, 1.08, 'b', transform=ax_hist_2.transAxes, 
...
plt.savefig(
    os.path.join(figures_dir, f"Figure_11_amip_vs_obs_marginal_sensitivity_hist.png"), 
...
len(np.where(metrics_gdf["correlation"]>.8)[0])/len(metrics_gdf["correlation"])


# In[22]:


len(np.where(metrics_gdf["correlation"]<0)[0])/len(metrics_gdf["correlation"])
