...
if USE_LONGER_PERIOD:
    PRECIP_DATASETS_TO_USE = ['CRU', 'CPC', 'PREC', 'TerraClimate']
    SST_DATASETS_TO_USE = ['ERSSTv5']
    TIME_PERIOD = "1980-2023"
else:
    PRECIP_DATASETS_TO_USE = ['GPCP', 'CRU', 'GPCC', 'CPC', 'UDel', 'PREC', 'TerraClimate', 'REGEN', 'MSWEP']
    SST_DATASETS_TO_USE = ['ERSSTv5', 'COBE-SST2']
...
def compute_pattern_corr_all_windows(full_beta_values, window_ensemble_stack):
    """
    Compute pattern correlation between full period and all windows
    for all ensemble members.
    
    Parameters:
    - full_beta_values: (lat, lon) - full period reference
    - window_ensemble_stack: (window, ensemble, lat, lon) - all windows and ensemble members
    
    Returns:
    - corr_means: (window,) - mean correlation across ensemble for each window
    - corr_stds: (window,) - std of correlation across ensemble for each window
    """
    full_flat = full_beta_values.flatten()
    n_windows = window_ensemble_stack.shape[0]
    n_ensemble = window_ensemble_stack.shape[1]
    
    corr_means = np.zeros(n_windows, dtype=np.float32)
    corr_stds = np.zeros(n_windows, dtype=np.float32)
    
    # Loop over windows
    for w_idx in range(n_windows):
        corrs = np.zeros(n_ensemble)
        
        # Loop over ensemble members for this window
        for e_idx in range(n_ensemble):
            ...
            # Compute correlation
            if len(full_valid) > 1:
                corrs[e_idx] = pearsonr(full_valid, window_valid)[0]
            else:
                corrs[e_idx] = np.nan
        
        corr_means[w_idx] = np.nanmean(corrs)
        corr_stds[w_idx] = np.nanstd(corrs)
    
    return corr_means, corr_stds
...
result = xr.apply_ufunc(
    ...
    vectorize=True,
    output_core_dims=[['window'], ['window']],
    output_dtypes=[np.float32, np.float32]
)

pattern_corr_means_da = result[0]
pattern_corr_stds_da = result[1]

print(f"Mean pattern correlation across all basins: {float(pattern_corr_means_da.mean()):.3f} ± {float(pattern_corr_stds_da.mean()):.3f}")
...
results_ds = xr.Dataset(
    {
        'pattern_corr_mean': pattern_corr_means_da,
        'pattern_corr_std': pattern_corr_stds_da,
        'full_period_beta': full_period_beta
    },
    coords={
