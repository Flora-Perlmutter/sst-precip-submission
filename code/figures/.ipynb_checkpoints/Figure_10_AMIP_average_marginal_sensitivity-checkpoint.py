...
quantities instead of observational ensemble quantities:
  Panel A: AMIP ensemble mean marginal sensitivity (dP/dSST)
  Panel B: AMIP convolved quantity: MS × SST variability
...
sst_cmap = ListedColormap(colors[:-1])  # colors for boxes only
sst_cmap.set_over(colors[-1])            # upper arrow
...
ms_cmap = ListedColormap(colors[:-1])  # colors for boxes only
ms_cmap.set_over(colors[-1])            # upper arrow
...
se_cmap = ListedColormap(colors[:-1])  # colors for boxes only
se_cmap.set_over(colors[-1])            # upper arrow
...
total_cmap = ListedColormap(colors[:-1])  # colors for boxes only
total_cmap.set_over(colors[-1])            # upper arrow
