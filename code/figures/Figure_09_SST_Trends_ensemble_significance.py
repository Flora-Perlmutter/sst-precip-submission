...
# coding: utf-8
"""
Figure 9: SST-forced precipitation trends (observed vs AMIP).
...
A basin is marked significant (unhatched) only when it passes BOTH gates:
  ...
  Gate 1 — t-test across member trends:
      ...
      N_INDEPENDENT is set conservatively:
        • Obs  : N_INDEPENDENT_OBS  = 8  (number of independent precip
                 datasets; the 16 members are 8 precip × 2 SST combinations
                 and the two SST datasets are highly correlated, so the
                 effective information is ~8, not 16)
  ...
  Gate 2 — sign agreement:
      ...
      • Obs  : threshold SIGN_THRESHOLD_OBS  = 0.75  (≥ 6/8 independent)
      • AMIP : threshold SIGN_THRESHOLD_AMIP = 0.67  (≥ 4/6 models)
...
Ensemble-size sensitivity test
--------------------------------
Appended after the figure.  Sub-samples the obs ensemble to N=6 (AMIP size)
5000 times to test whether the difference in significant basin counts between
obs and AMIP is explained by ensemble size alone.
...
SIGN_THRESHOLD_OBS  = 0.75  # ≥ 6/8 independent obs members
SIGN_THRESHOLD_AMIP = 0.67  # ≥ 4/6 AMIP models
...
ax5.set_title("Observed SST-Forced Precipitation Trend")
ax5.set_global()
...
ax6.set_title("AMIP SST-Forced Precipitation Trend")
ax6.set_global()
...
ax7.set_xlabel(r"Observed SST-Forced Trend (mm month$^{-1}$ decade$^{-1}$)")
...
ax8.set_xlabel(r"AMIP SST-Forced Trend (mm month$^{-1}$ decade$^{-1}$)")
...
plt.savefig(FIGURES_DIR / "Figure_9_SST_forced_trends.png",
